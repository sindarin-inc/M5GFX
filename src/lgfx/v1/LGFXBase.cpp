...
void LGFXBase::fillCircleHelper(int32_t x, int32_t y, int32_t r, uint_fast8_t corners,
                                int32_t delta) {
    if (r <= 0) return;
...
void LGFXBase::drawTriangle(int32_t x0, int32_t y0, int32_t x1, int32_t y1, int32_t x2,
                            int32_t y2) {
    startWrite();
...
void LGFXBase::fillTriangle(int32_t x0, int32_t y0, int32_t x1, int32_t y1, int32_t x2,
                            int32_t y2) {
    int32_t a, b;
    ...
    int32_t err1 = (std::max(dx1, dy1) >> 1) + (xstep1 < 0 ? std::min(dx1, dy1) : dx1);
    int32_t err2 = (std::max(dx2, dy2) >> 1) + (xstep2 > 0 ? std::min(dx2, dy2) : dx2);
    startWrite();
    if (y0 != y1) {
        do {
            err1 -= dx1;
    ...
    do {
        ...
        while (err1 < 0) {
            err1 += dy1;
            if ((a += xstep1) == x2) break;
        }
        err2 -= dx2;
        while (err2 < 0) {
            err2 += dy2;
            if ((b += xstep2) == x2) break;
        }
        writeFastHLine(a, y0, b - a + 1);
    } while (++y0 <= y2);
    endWrite();
...
void LGFXBase::draw_bezier_helper(int32_t x0, int32_t y0, int32_t x1, int32_t y1, int32_t x2,
                                  int32_t y2) {
...
bool LGFXBase::clampArea(int32_t *xlo, int32_t *ylo, int32_t *xhi, int32_t *yhi) {
    if ((*xlo >= width()) || (*ylo >= height()))
        return false;                           // lowest coords are outside (higher) of viewport
...
rgb888_t LGFXBase::map_gradient(float value, float start, float end, const rgb888_t *colors,
                                uint32_t colors_count) {
    if (!colors) return rgb888_t(0, 0, 0);
    if (colors_count <= 1)
        return colors[0]; // there's no point mapping to a single value, also avoid divide by zero
    ...
    if (distance < min_precision) {
        ...
        return rgb888_t(r1 + distance * float(r2 - r1), g1 + distance * float(g2 - g1),
                        b1 + distance * float(b2 - b1));
    }
}
...
void LGFXBase::draw_gradient_line(int32_t x0, int32_t y0, int32_t x1, int32_t y1,
    ...
    for (int32_t x = x0; x <= x1; x++) {
        setColor(color888((x - x0) * diff_r / dx + r, (x - x0) * diff_g / dx + g,
                          (x - x0) * diff_b / dx + b));
        if (steep)
            writePixel(y0, x);
        else
...
void LGFXBase::draw_gradient_wedgeline(float ax, float ay, float bx, float by, float ar, float br,
                                       const colors_t gradient) {
    ...
    for (int32_t yp = ys; yp <= y1; yp++) {
        ...
        for (int32_t xp = xs; xp <= x1; xp++) {
            ...
            // handle gradient
            if (gradient.count > 1)
                fg_color = map_gradient(pixelDistance(ax, ay, xp, yp), 0.0f, linedist, gradient);
    ...
    for (int32_t yp = ys - 1; yp >= y0; yp--) {
        ...
        for (int32_t xp = xs; xp <= x1; xp++) {
            if (endX)
                if (alpha <= LoAlphaTheshold) break; // Skip right side of drawn line
            xpax = xp - ax;
            alpha = ar - wedgeLineDistance(xpax, ypay, bax, bay, rdt);
            if (alpha <= LoAlphaTheshold) continue;
            // handle gradient
...
void LGFXBase::draw_wedgeline(float ax, float ay, float bx, float by, float ar, float br,
...
void LGFXBase::fill_rect_radial_gradient(int32_t x, int32_t y, uint32_t w, uint32_t h,
                                         const colors_t gradient) {
...
void LGFXBase::fill_rect_radial_gradient(int32_t x, int32_t y, uint32_t w, uint32_t h,
                                         const uint32_t colorstart, const uint32_t colorend) {
    const rgb888_t colors[2] = {colorstart, colorend};
    auto gradient = createGradient(colors);
    fill_rect_radial_gradient(x, y, w, h, gradient);
}

void LGFXBase::fill_rect_linear_gradient(int32_t x, int32_t y, uint32_t w, uint32_t h,
                                         const colors_t gradient, fill_style_t style) {
    if (!gradient.colors || gradient.count == 0) return;
...
void LGFXBase::fill_rect_gradient(int32_t x, int32_t y, uint32_t w, uint32_t h,
                                  const colors_t gradient, fill_style_t style) {
    style == RADIAL ? fill_rect_radial_gradient(x, y, w, h, gradient)
...
void LGFXBase::drawEllipseArc(int32_t x, int32_t y, int32_t r0x, int32_t r1x, int32_t r0y,
                              int32_t r1y, float start, float end) {
...
void LGFXBase::fillEllipseArc(int32_t x, int32_t y, int32_t r0x, int32_t r1x, int32_t r0y,
                              int32_t r1y, float start, float end) {
    if (r0x < r1x) std::swap(r0x, r1x);
    if (r0y < r1y) std::swap(r0y, r1y);
    if (r1x < 0) return;
...
void LGFXBase::fill_arc_helper(int32_t cx, int32_t cy, int32_t oradius_x, int32_t iradius_x,
                               int32_t oradius_y, int32_t iradius_y, float start, float end) {
    ...
    do {
        ...
        do {
            ...
            int32_t x2 = x * x;
            if (x2 >= compare_i && ((flg1 && flg2) || (reversed && (flg1 || flg2))) && x != xe &&
                x2 < compare_o) {
                ++len;
            } else {
                if (len) {
...
void LGFXBase::draw_bitmap(int32_t x, int32_t y, const uint8_t *bitmap, int32_t w, int32_t h,
                           uint32_t fg_rawcolor, uint32_t bg_rawcolor) {
    if (w < 1 || h < 1) return;
...
void LGFXBase::draw_xbitmap(int32_t x, int32_t y, const uint8_t *bitmap, int32_t w, int32_t h,
                            uint32_t fg_rawcolor, uint32_t bg_rawcolor) {
    if (w < 1 || h < 1) return;
    setRawColor(fg_rawcolor);
    int32_t byteWidth = (w + 7) >> 3;
...
pixelcopy_t LGFXBase::create_pc_gray(const uint8_t *image, lgfx::color_depth_t depth,
...
void LGFXBase::push_grayimage(int32_t x, int32_t y, int32_t w, int32_t h, const uint8_t *image,
                              color_depth_t depth, uint32_t fore_rgb888, uint32_t back_rgb888) {
...
void LGFXBase::push_grayimage_rotate_zoom(float dst_x, float dst_y, float src_x, float src_y,
                                          float angle, float zoom_x, float zoom_y, int32_t w,
                                          int32_t h, const uint8_t *image, color_depth_t depth,
                                          uint32_t fore_rgb888, uint32_t back_rgb888) {
    pixelcopy_t pc = create_pc_gray(image, depth, fore_rgb888, back_rgb888);
    push_image_rotate_zoom(dst_x, dst_y, src_x, src_y, angle, zoom_x, zoom_y, w, h, &pc);
}

void LGFXBase::push_grayimage_affine(const float *matrix, int32_t w, int32_t h,
                                     const uint8_t *image, color_depth_t depth,
                                     uint32_t fore_rgb888, uint32_t back_rgb888) {
    pixelcopy_t pc = create_pc_gray(image, depth, fore_rgb888, back_rgb888);
    push_image_affine(matrix, w, h, &pc);
}

void LGFXBase::pushImage(int32_t x, int32_t y, int32_t w, int32_t h, pixelcopy_t *param,
                         bool use_dma) {
...
void LGFXBase::make_rotation_matrix(float *result, float dst_x, float dst_y, float src_x,
                                    float src_y, float angle, float zoom_x, float zoom_y) {
...
void LGFXBase::push_image_rotate_zoom(float dst_x, float dst_y, float src_x, float src_y,
                                      float angle, float zoom_x, float zoom_y, int32_t w, int32_t h,
                                      pixelcopy_t *pc) {
    float matrix[6];
    make_rotation_matrix(matrix, dst_x + 0.5f, dst_y + 0.5f, src_x + 0.5f, src_y + 0.5f, angle,
                         zoom_x, zoom_y);
    push_image_affine(matrix, w, h, pc);
}

void LGFXBase::push_image_rotate_zoom_aa(float dst_x, float dst_y, float src_x, float src_y,
                                         float angle, float zoom_x, float zoom_y, int32_t w,
                                         int32_t h, pixelcopy_t *pc) {
    float matrix[6];
    make_rotation_matrix(matrix, dst_x + 0.5f, dst_y + 0.5f, src_x + 0.5f, src_y + 0.5f, angle,
                         zoom_x, zoom_y);
...
void LGFXBase::push_image_affine(const float *matrix, pixelcopy_t *pc) {
    ...
    do {
        ...
        int32_t left = std::max(
            cl, std::max(iA[0] ? (iA[2] + xs1) / -iA[0] : cl, iA[3] ? (iA[5] + ys1) / -iA[3] : cl));
        int32_t right = std::min(
            cr, std::min(iA[0] ? (iA[2] + xs2) / -iA[0] : cr, iA[3] ? (iA[5] + ys2) / -iA[3] : cr));
        if (left < right) {
...
void LGFXBase::push_image_affine_aa(const float *matrix, pixelcopy_t *pc, pixelcopy_t *pc2) {
    ...
    pc->src_y32_add = iA[3];
    uint32_t x32_diff =
        std::min<uint32_t>(8 << FP_SCALE, std::max(abs(iA[0]), abs(iA[1])) - 1) >> 1;
    uint32_t y32_diff =
        std::min<uint32_t>(8 << FP_SCALE, std::max(abs(iA[3]), abs(iA[4])) - 1) >> 1;
...
void LGFXBase::copyRect(int32_t dst_x, int32_t dst_y, int32_t w, int32_t h, int32_t src_x,
                        int32_t src_y) {
    auto wid = width();
    if (src_x < dst_x) {
        if (src_x < 0) {
...
void LGFXBase::read_rect(int32_t x, int32_t y, int32_t w, int32_t h, void *dst,
                         pixelcopy_t *param) {
    _adjust_abs(x, w);
    if (x < 0) {
        w += x;
...
struct paint_point_t {
    int32_t lx, rx, y, oy;
};

static void paint_add_points(std::list<paint_point_t> &points, int32_t lx, int32_t rx, int32_t y,
                             int32_t oy, uint8_t *linebuf) {
...
void LGFXBase::floodFill(int32_t x, int32_t y) {
    ...
    case color_depth_t::rgb888_3Byte:
        p.fp_copy = pixelcopy_t::compare_rgb_affine<bgr888_t>;
        break;
    case color_depth_t::rgb666_3Byte:
        p.fp_copy = pixelcopy_t::compare_rgb_affine<bgr666_t>;
        break;
    case color_depth_t::rgb565_2Byte:
        p.fp_copy = pixelcopy_t::compare_rgb_affine<swap565_t>;
        break;
    case color_depth_t::rgb332_1Byte:
        p.fp_copy = pixelcopy_t::compare_rgb_affine<rgb332_t>;
        break;
    default:
        p.fp_copy = pixelcopy_t::compare_bit_affine;
    ...
    while (!points.empty()) {
        ...
        while (lx > cl && linebuf[lx - 1])
            --lx;
        while (rx < cr && linebuf[rx + 1])
            ++rx;
...
static char *floatToStr(double number, char *buf, size_t /*buflen*/, uint8_t digits) {
    if (isnan(number)) {
        return (char *)memcpy(buf, "nan\0", 4);
    }
    if (isinf(number)) {
        return (char *)memcpy(buf, "inf\0", 4);
    }
    if (number > 4294967040.0) {
        return (char *)memcpy(buf, "ovf\0", 4);
...
int32_t LGFXBase::textLength(const char *string, int32_t width) {
    ...
    do {
        ...
        if (left == 0 && right == 0 && _font_metrics.x_offset < 0)
            left = right = -((_font_metrics.x_offset * sx) >> 16);
        int32_t sxadvance = (_font_metrics.x_advance * sx) >> 16;
        right = left + std::max<int>(sxadvance, ((_font_metrics.width * sx) >> 16) +
                                                    ((_font_metrics.x_offset * sx) >> 16));
        // right = left + (int)(std::max<int>(_font_metrics.x_advance, _font_metrics.width +
        // _font_metrics.x_offset) * sx);
        left += sxadvance;
...
int32_t LGFXBase::text_width(const char *string, const IFont *font, FontMetrics *metrics) {
    ...
    do {
        ...
        // right = left + (int)(std::max<int>(metrics->x_advance, metrics->width +
        // metrics->x_offset) * sx);
        left += sxadvance;
    } while (*(++string));
    return right;
...
size_t LGFXBase::draw_string(const char *string, int32_t x, int32_t y, textdatum_t datum,
    ...
    int16_t sumX = 0;
    int32_t cwidth =
        text_width(string, font, &metrics); // Find the pixel width of the string in the font
    int32_t sy = 65536 * _text_style.size_y;
    ...
    if (string && string[0]) {
        do {
            ...
            sumX +=
                font->drawChar(this, x + sumX, y, uniCode, &_text_style, &metrics, dummy_filled_x);
        } while (*(++string));
    }
    this->endWrite();
...
size_t LGFXBase::write(uint8_t utf8) {
    ...
    if (utf8 == '\n') {
        ...
        int32_t w =
            std::max(xo + ((_font_metrics.width * sx) >> 16), (_font_metrics.x_advance * sx) >> 16);
        ...
        if (y <= _clip_b + h) {
            _cursor_x += _font->drawChar(this, _cursor_x, y, uniCode, &_text_style, &_font_metrics,
                                         _filled_x);
        } else {
            _font->updateFontMetric(&_font_metrics, uniCode);
...
uint8_t LGFXBase::getAttribute(attribute_t attr_id) {
    switch (attr_id) {
    case cp437_switch:
        return _text_style.cp437;
    case utf8_switch:
        return _text_style.utf8;
    case epd_mode_switch:
        return _panel->getEpdMode();
    default:
        return 0;
    }
}
...
public:
    void end(void) { gfx->setClipRect(_cl, _ct, _cw, _ch); }

    bool begin(LGFXBase *gfx_, int32_t x_, int32_t y_, int32_t maxWidth_, int32_t maxHeight_,
               int32_t offX_, int32_t offY_, float zoom_x_, float zoom_y_, datum_t datum_,
        ...
        if (zoom_y_ <= 0.0f || zoom_x_ <= 0.0f) {
            ...
            if (zoom_x_ <= -1.0f) {
                zoom_x_ = fit_width / w_;
            }
            if (zoom_y_ <= -1.0f) {
                zoom_y_ = fit_height / h_;
            }
            if (zoom_x_ <= 0.0f) {
                if (zoom_y_ <= 0.0f) {
        ...
        if (maxHeight_ > bottom - y_) {
            maxHeight_ = bottom - y_;
        }
...
bool LGFXBase::draw_bmp(DataWrapper *data, int32_t x, int32_t y, int32_t maxWidth,
                        int32_t maxHeight, int32_t offX, int32_t offY, float zoom_x, float zoom_y,
                        datum_t datum) {
    ...
    image_info_t info;
    if (!info.begin(this, x, y, maxWidth, maxHeight, offX, offY, zoom_x, zoom_y, datum, w, h)) {
        return true;
    }
    ...
    do {
        ...
        } else if (bmpdata.biCompression == 2) {
...
static uint32_t jpg_push_image(void *device, void *bitmap, JRECT *rect) {
    ...
    jpeg->gfx->pushImage(jpeg->x + x, jpeg->y + y, w, h, jpeg->pc, false);
    return 1;
}
...
static uint32_t jpg_push_image_affine(void *device, void *bitmap, JRECT *rect) {
    ...
    int32_t w = rect->right - rect->left + 1;
    int32_t h = rect->bottom - rect->top + 1;
    float affine[6] = {jpeg->zoom_x, 0.0f,         x * jpeg->zoom_x + jpeg->x,
                       0.0f,         jpeg->zoom_y, y * jpeg->zoom_y + jpeg->y};
    jpeg->gfx->pushImageAffine(affine, w, h, (bgr888_t *)jpeg->pc->src_data);
    return 1;
}

bool LGFXBase::draw_jpg(DataWrapper *data, int32_t x, int32_t y, int32_t maxWidth,
                        int32_t maxHeight, int32_t offX, int32_t offY, float zoom_x, float zoom_y,
                        datum_t datum) {
    prepareTmpTransaction(data);
    ...
    if (!drawinfo.begin(this, x, y, maxWidth, maxHeight, offX, offY, zoom_x, zoom_y, datum,
    ...
    if (drawinfo.offX) {
        drawinfo.x -= drawinfo.offX;
        drawinfo.offX = 0;
    }
    if (drawinfo.offY) {
        drawinfo.y -= drawinfo.offY;
        drawinfo.offY = 0;
    }
    ...
    if (scale_max <= 0.5f) {
        if (scale_max <= 0.125f) {
            div = jpeg_div::jpeg_div_t::JPEG_DIV_8;
        } else if (scale_max <= 0.25f) {
            div = jpeg_div::jpeg_div_t::JPEG_DIV_4;
        } else {
            div = jpeg_div::jpeg_div_t::JPEG_DIV_2;
        }
    ...
    jres = lgfx_jd_decomp(
        &jpegdec,
        drawinfo.zoom_x == 1.0f && drawinfo.zoom_y == 1.0f ? jpg_push_image : jpg_push_image_affine,
        div);
...
static void png_draw_alpha_callback(void *user_data, uint32_t x, uint32_t y, uint_fast8_t div_x,
...
static void png_draw_alpha_scale_callback(void *user_data, uint32_t x, uint32_t y,
                                          uint_fast8_t div_x, size_t len, const uint8_t *argb) {
    auto p = (png_file_decoder_t *)user_data;
                        ...
                        if (a) {
                            if (a == 255) {
                                ...
                                do {
                                    ...
                                    data->set((ar + data->r * inv) >> 8, (ag + data->g * inv) >> 8,
                                              (ab + data->b * inv) >> 8);
                                } while (++l < r);
                            }
                        }
...
bool LGFXBase::draw_png(DataWrapper *data, int32_t x, int32_t y, int32_t maxWidth,
                        int32_t maxHeight, int32_t offX, int32_t offY, float zoom_x, float zoom_y,
                        datum_t datum) {
    ...
    if (!png.begin(this, x, y, maxWidth, maxHeight, offX, offY, zoom_x, zoom_y, datum,
    ...
    auto res = lgfx_pngle_decomp(pngle, png.zoom_x == 1.0f && png.zoom_y == 1.0f
...
bool LGFXBase::draw_qoi(DataWrapper *data, int32_t x, int32_t y, int32_t maxWidth,
                        int32_t maxHeight, int32_t offX, int32_t offY, float zoom_x, float zoom_y,
                        datum_t datum) {
    ...
    auto res = lgfx_qoi_decomp(qoi, png.zoom_x == 1.0f && png.zoom_y == 1.0f
                                        ? png_draw_alpha_callback
                                        : png_draw_alpha_scale_callback);
...
static uint8_t *png_encoder_get_row(uint8_t *pImage, int flip, int w, int h, int y, int,
                                    void *target) {
...
void *LGFXBase::createPng(size_t *datalen, int32_t x, int32_t y, int32_t w, int32_t h) {
    ...
    auto res = m5_m5_tdefl_write_image_to_png_file_in_memory_ex_with_cb(
        rgbBuffer, w, h, 3, datalen, 6, 0, (tdefl_get_png_row_func)png_encoder_get_row, &enc);
...
LGFX_Device::LGFX_Device(void) { setPanel(nullptr); }

void LGFX_Device::initBus(void) { panel()->initBus(); };

void LGFX_Device::releaseBus(void) { panel()->releaseBus(); };
...
void LGFX_Device::draw_calibrate_point(int32_t x, int32_t y, int32_t r, uint32_t fg_rawcolor,
                                       uint32_t bg_rawcolor) {
...
void LGFX_Device::calibrate_touch(uint16_t *parameters, uint32_t fg_rawcolor, uint32_t bg_rawcolor,
                                  uint8_t size) {
    ...
    for (int i = 0; i < 4; ++i) {
        ...
        for (int j = 0; j < 8; ++j) {
            do {
                do {
                    delay(2);
                } while (!getTouchRaw(&tp));
                delay(10);
            } while (!getTouchRaw(&tp2) || (abs(tp.x - tp2.x) > _RAWERR) ||
